...
3. Hệ thống hiển thị sơ đồ trực quan, sử dụng màu sắc phân biệt (xanh lá = trống, xanh dương = đang có khách, vàng = đặt trước, đỏ = chờ dọn).\newline
4. Nhân viên xem thông tin, có thể chạm vào một bàn để xem chi tiết (thời gian đã ngồi, nhân viên phụ trách). \\ \hline
\textbf{Alternative Flows} &
\textbf{1a. Lỗi kết nối mạng:} Hệ thống hiển thị thông báo ``Không thể tải sơ đồ bàn. Vui lòng kiểm tra lại kết nối'' và cung cấp nút ``Thử lại''. \\ \hline
\end{longtable}
...
\textbf{Alternative Flows} &
\textbf{3a. Đánh dấu nhầm món:} Nhân viên nhấn nút ``Hoàn tác'' (Undo) ngay cạnh món ăn vừa đánh dấu trong 30 giây. Hệ thống đưa món về trạng thái ``Sẵn sàng phục vụ''. \\ \hline
\end{longtable}
...
\textbf{Description} & Cung cấp thông tin ước lượng thời gian chờ (ETA) liên tục và chính xác nhất cho khách hàng dựa trên số lượng bàn bận và tiến độ ăn của khách hiện tại. \\ \hline
\textbf{Trigger} & Hệ thống tự động kích hoạt mỗi khi có bàn thay đổi trạng thái hoặc định kỳ mỗi 5 phút. \\ \hline
\textbf{Preconditions} &
$\bullet$ Có khách hàng đang nằm trong danh sách chờ.\newline
$\bullet$ Hệ thống ghi nhận thời điểm khách bắt đầu ngồi vào bàn (seated\_at). \\ \hline
\textbf{Postconditions} & $\bullet$ Thời gian chờ dự kiến của mỗi khách được cập nhật và hiển thị theo thời gian thực trên màn hình Host. \\ \hline
\textbf{Normal Flow} &
1. Hệ thống định kỳ quét trạng thái tất cả các bàn đang bận.\newline
2. Hệ thống tính toán lại ETA dựa trên số bàn đang có khách và vị trí của khách trong hàng chờ.\newline
3. Hệ thống tự động làm mới cột ``Thời gian chờ dự kiến'' và cập nhật màu sắc (Xanh $\le$ 5p, Vàng $\le$ 15p).\newline
4. Tiếp tân theo dõi thông tin để thông báo cho khách hàng. \\ \hline
\textbf{Alternative Flows} &
\textbf{2a. Bàn hiện tại kéo dài quá giờ:} Hệ thống phát hiện bàn ngồi quá thời gian quy định (vd: $>$ 60 phút), tự động cộng dồn thời gian trễ vào danh sách chờ và hiển thị cảnh báo nổi bật để Tiếp tân chú ý xin lỗi khách. \\ \hline
\end{longtable}
...
\textbf{Description} & Hiển thị các phiếu gọi món (tickets) trực quan, phân loại theo trạm bếp và sắp xếp theo độ ưu tiên (VIP, thời gian chờ) kèm theo trạng thái đã phục vụ. \\ \hline
\textbf{Trigger} & Nhân viên phục vụ chốt đơn và nhấn ``Gửi Bếp'' hoặc có cập nhật trạng thái từ phía phục vụ. \\ \hline
\textbf{Preconditions} & $\bullet$ Hệ thống KDS đang hoạt động và kết nối thời gian thực với máy chủ. \\ \hline
\textbf{Postconditions} & $\bullet$ Đầu bếp nắm bắt được danh sách món cần làm và theo dõi được các món đã ra bàn thành công. \\ \hline
\textbf{Normal Flow} &
1. Hệ thống KDS nhận dữ liệu đơn hàng mới từ máy chủ qua SSE.\newline
2. Hệ thống tự động tách và phân bổ món về các màn hình trạm (Nướng, Chiên, Bar...). \newline
3. Hệ thống hiển thị các phiếu món ăn, ưu tiên món khách VIP lên đầu hàng chờ.\newline
4. Hệ thống hiển thị cả các món đã phục vụ (Served) với màu sắc riêng biệt để đầu bếp kiểm soát tiến độ tổng thể. \\ \hline
\textbf{Alternative Flows} &
\textbf{3a. Đơn hàng bị trễ SLA:} Hệ thống KDS tự động nhấp nháy viền đỏ và đổi màu nền để đầu bếp ưu tiên xử lý món đó ngay. \\ \hline
\end{longtable}
